...
lang_init() {
  ...
  Loop Files, directory "\*.ini"
  {
    ...
    if (enlangname != "Error" && langname != "Error" && code != "Error")
    {
      ...
      _language.allLangs[code].filepath := A_LoopFilePath
  ...
  if (not _language.allLangs[_language.fallbacklang])
    MsgBox("Fallback language not found in directory " directory, "lang()", 16)
...
lang_setLanguage(p_lang := "") {
  ...
  if (not _language.lang_init_called) {
    MsgBox("Bug detected! lang_setLanguage() must becalled before lang_init()", "lang()", 16)
    return
  }
  ...
  if (p_lang != "") {
    ...
    if (not lang) {
      MsgBox("Language `"" p_lang "`" is invalid.", "lang()", 16)
      return
    }
  }
  ...
  if (not lang) {
    ...
    for onecode, onelang in _language.allLangs {
      ...
      if InStr(syslang, templangenname) {
...
lang(langvar, $1 := "", $2 := "", $3 := "", $4 := "", $5 := "", $6 := "", $7 := "", $8 := "", $9 := "") {
  ...
  if (not _language.lang_init_called) {
    MsgBox("Bug detected! lang_setLanguage() must becalled before lang()", "lang()", 16)
    return
  }
  ...
  if ((!(NoCache)) and (_language.cache.Has(langvar))) {
  ...
  if (initext="") {
    ;read from ini file
    iniAllSections := IniRead(filepath)
    Loop Parse, iniAllSections, "`n"
    {
      initext := IniRead(filepath, a_loopfield, langvar, A_Space)
      if initext {
        _language.success := 1
        break
      }
  ...
  if (initext="") {
    iniAllSections := IniRead(fallbackfilepath)
    Loop Parse, iniAllSections, "`n" {
      initext := IniRead(fallbackfilepath, a_loopfield, langvar, A_Space)
      if initext {
        _language.success := 0
        break
      }
  ...
  initext := StrReplace(initext, "%1%", $1)
  initext := StrReplace(initext, "%2%", $2)
  initext := StrReplace(initext, "%3%", $3)
  initext := StrReplace(initext, "%4%", $4)
  initext := StrReplace(initext, "%5%", $4)
  initext := StrReplace(initext, "%6%", $4)
  initext := StrReplace(initext, "%7%", $4)
  initext := StrReplace(initext, "%8%", $4)
  initext := StrReplace(initext, "%9%", $4)
...
lang_ReadAllTranslations() {
  ...
  Loop read, filepath {
    ...
    if InStr(a_loopreadline, "=") {
      ...
      tempItemName := SubStr(a_loopreadline, 1, pos)
      
      tempItemContent := SubStr(a_loopreadline, ((pos +1))+1)
