...
selectAttachmentFile(*){
  ...
  if (!selectedIsValid(currentLineNumber)){
    ...
    selectedFile := FileSelect("1", extractPath(attachment), "Bitte eine Datei auswählen! (bei mehreren Dateien: Pfad auswählen verwenden oder den Pfad manuell im Feld `"Anhang:`" eintragen!)")

    if (selectedFile != ""){
      effectivFP := selectedFile
      selectedFile := pathToRelativ(selectedFile, extractPath(attachment))
...
selectAttachmentPath(*){
  global
  local selectedPath, extractedFN, extractedP, effectivFP
  
  effectivFP := ""
  if (!selectedIsValid(currentLineNumber)){
    showHintColored("Bitte zunächst einen Eintrag auswählen!")
  } else {
    selectedPath := FileSelect("D", extractPath(attachment), "Bitte den Pfad auswählen! (bei mehreren Dateien): oder den Pfad manuell im Feld `"Anhang:`" eintragen!)")

    if (selectedPath != ""){
      selectedPath := pathToRelativ(selectedPath, extractPath(attachment))
