...
selectAttachmentFile(*){
  ...
  if (!selectedIsValid(currentLineNumber)){
    ...
    selectedFile := FileSelect("1", attachmentsPathInUse, "Bitte eine Datei auswählen! (bei mehreren Dateien: Pfad auswählen verwenden oder den Pfad manuell im Feld `"Anhang:`" eintragen!)")
...
selectAttachmentPath(*){
  ...
  if (!selectedIsValid(currentLineNumber)){
    ...
    selectedPath := FileSelect("D", attachmentsPathInUse, "Bitte den Pfad auswählen! (bei mehreren Dateien): oder den Pfad manuell im Feld `"Anhang:`" eintragen!)")

    if (selectedPath != ""){
      selectedPath := pathToRelativ(selectedPath, attachmentsPathInUse)
